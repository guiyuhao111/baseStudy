...
function ConfirmDelivery(obj) {
    ...
    if (CheckedComButton(obj)) {
        var sysID = $("#showInfo").data("id");
        var logisticalName = $("#logisticalName").val();
        var logisticalNumber = $("#logisticalNumber").val();
        if (ValIsDefine(sysID) && ValIsDefine(logisticalName) && ValIsDefine(logisticalNumber)) {
            BtnAbled(obj);
            var url = "/web/web_ajax.aspx";
            var args = { "jsonType": "changeGoodsForCustomer", "id": sysID, "name":logisticalName,"number":logisticalNumber,"createTime": new Date() };
            $.post(url, args, function (data) {
                if (data != "UNLOGIN") {
                    if (data == "SUCCESS") {
                        ShowSuccess(obj);
            ...
            UnCheckedComButton(obj, html, "提交的数据不正确");
        }
    }
    return false;
}
