...
namespace DSMTMALL.Core.Common
{
    ...
    public class MetarnetRegex
    {
        ...
        public static bool IsMobilePhone(string input)
        {
            ...
            Regex dregex = new Regex(dianxin);
            //联通手机号正则        
            string liantong = @"^1[34578][01256]\d{8}$";
            Regex lregex = new Regex(liantong);
            //移动手机号正则        
            string yidong = @"^(134[012345678]\d{7}|1[34578][012356789]\d{8})$";
            Regex yregex = new Regex(yidong);
            if (dregex.IsMatch(input) || lregex.IsMatch(input) || yregex.IsMatch(input))
            {
                return true;
            }
            return false;
        ...
        /// </summary>
        /// <param name="pattern">模式字符串(正则表达式)</param>
        /// <param name="input">输入字符串</param>
        /// <returns></returns>
